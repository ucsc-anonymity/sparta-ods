...
private:
    ...
    int max(int a, int b);
    Node *newNode(Bid key, vector<byte_t> value);
    Node *newNode(Bid key);
...
public:
    ...
    Bid insert(Bid rootKey, unsigned long long &pos, Bid key, vector<byte_t> value, int &height, Bid lastID, bool isDummyIns = false);
    Bid insert2(Bid rootKey, unsigned long long &pos, Bid key, vector<byte_t> value, int &height, Bid lastID, bool isDummyIns = false);
    ...
    vector<byte_t> search(Node *head, Bid key);
